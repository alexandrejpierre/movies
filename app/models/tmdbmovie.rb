class Tmdbmovie < ActiveRecord::Base
	...
	# 20170602: filtered out the blank fr overviews
	def self.relevant_movies
		where.not(overview: nil).where.not(vote_average: 0.0).where.not(poster_path: nil).where.not(production_countries: nil).where.not(actors: nil).where.not(overview_fr: nil)
	end
end
