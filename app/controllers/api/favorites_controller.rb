module Api
    class FavoritesController < ApplicationController
		...
		def update
			...
			@status=Preference.where("user_id = ?",update_favorites_params[:user_id]).find_by("tmdbmovie_id = ?",update_favorites_params[:id]).update_attributes(favorite: update_favorites_params[:favorite])
